 ...
 */
@RestController
@RequestMapping("/app/rest/question/")
public class QuestionWebService {

    private static final Log log = LogFactory.getLog(QuestionWebService.class);
    ...
    @Timed
    public ResponseEntity<PaginationViewer> pagination(@RequestBody QuestionDTO dto,
                                                       @PathVariable("pageCount") Integer pageCount,
                                                       @PathVariable("pageSize") Integer pageSize) {

        List<QuestionDTO> uds = service.findAll(dto);
        if (uds.size() == 0)
    ...
    @RequestMapping(
            ...
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_ATOM_XML_VALUE})
    @Timed
    public ResponseEntity<QuestionDTO> saveOrUpdate(@RequestBody QuestionDTO dto) throws Exception {
        TranslationUtils.putTranslation(dto.getAnswer());
        TranslationUtils.putTranslation(dto.getQuestion());
